$(document).on("pageinit", function(e){
	...
	$(window).on('resize', function(e){
		if (sm.data('slideopen')) {
			...
			$(":jqmData(role='page')").css('left', '100%');
		}
	});
...
function slidemenu(sm, only_close) {
	...
	if (!sm.data('slideopen') && !only_close) {
		sm.show().animate({width: '100%', avoidTransforms: false, useTranslate3d: true}, 'fast');
		$(".ui-page-active").css('left', '100%');
		sm.data('slideopen', true);
		if ($(".ui-page-active :jqmData(role='header')").data('position') == 'fixed') {
			$(".ui-page-active :jqmData(slidemenu)").css('margin-left', '0');
			$('.ui-btn-inner').addClass('acmenu');
			$('.back_menu').show('slow');
			$('.fix_top').css('overflow','visible ');
		...
		$('.ui-btn-inner').removeClass('acmenu');
		$('.back_menu').hide('slow');
		$('.fix_top').css('overflow','hidden ');
